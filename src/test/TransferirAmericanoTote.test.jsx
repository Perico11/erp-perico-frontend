/* ════════════════════════════════════════════════════════════════════════════
   ELEGIR EL TAMBO AL TRANSFERIR (21-ago-2026).
   ...
   Lo que se fija aquí es que el tambo elegido SEA el que se manda, que no se
   pueda pedir de un tambo más de lo que tiene, y que "automático" siga
...
/* El Almacén 2 tal como estaba el 21-ago: varios tambos idénticos de 1 000 L,
...
describe('elegir el tambo al transferir litros', () => {
  it('lista los tambos con su lote de proveedor y sus litros', () => {
    abrir();
  ...
  it('no ofrece tambos vacíos: no se puede cargar lo que no tiene nada', () => {
    abrir();
    const vals = [...selector().options].map(o => o.value);
    expect(vals).not.toContain('USA2-0009-99');
  });

  it('manda EL tambo elegido, no otro de los iguales', async () => {
    /* El fallo original en una línea: dos tambos de 1 000 L y el sistema
       escogía el primero. */
    abrir();
  ...
  it('al elegir un tambo precarga sus litros — mover uno completo es lo normal', () => {
    abrir();
    fireEvent.change(selector(), { target: { value: 'USA2-0009-03' } });
    expect(cantidad().value).toBe('981');
  });

  it('el techo pasa a ser ESE tambo, no el granel del color', async () => {
    /* Pedir 1 500 L de un tambo de 981 saldría de otro tote: justo el problema
  ...
  it('sin elegir tambo NO manda codigoLote — el granel suelto sigue funcionando', async () => {
    abrir();
  ...
  it('el resumen dice de qué tambo sale, para poder cotejarlo con el fisico', () => {
    abrir();
    fireEvent.change(selector(), { target: { value: 'GD89563' } });
  ...
  it('cambiar a cubetas quita el selector y olvida el tambo', async () => {
    /* Un tambo elegido que sobreviva a un cambio de presentación mandaria un
