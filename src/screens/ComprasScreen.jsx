...
// Claude Design (cards en grid). SOLO contenido + estado de tab; cero fetch/lógica.
// Props: data{ocs:[{cod,mp,qty,sol?,prov,monto,entrega,estado,vencida?,porVencer?,pago?,comp?}]},
//   onNuevaOC, onAprobarOC(cod), onEditarOC(cod), onEliminarOC(cod), onRecibirMP(cod),
//   onRegistrarPago(cod), onImprimirOC(cod), can, role, isDesktop.
// NOTA (integración): "Aprobar OC" llama onAprobarOC(cod) → abre el modal REAL de la app
// (AprobarOCModal con pago + comprobante + vencimiento). NO se usa un sheet inline aquí para
// no perder la subida real del comprobante (el sheet del mockup solo capturaba el nombre).
import { useState } from 'react';

const DEMO = { ocs: [
  { cod:'OC-2050', mp:'Dióxido de titanio', qty:'500 kg', sol:'Enrique', prov:'Química Tlaloc', monto:'$185,000', entrega:'10 jun', estado:'porAprobar' },
  { cod:'OC-2041', mp:'Resina acrílica', qty:'800 kg', prov:'Químicos del Bajío', monto:'$112,000', entrega:'31 may', estado:'activa', vencida:'4 días' },
  { cod:'OC-2038', mp:'Carbonato de calcio', qty:'1,200 kg', prov:'Minerales del Centro', monto:'$31,500', entrega:'02 jun', estado:'recibida' },
] };
...
function btn(kind) {
  ...
  if (kind === 'ghost') return { ...base, flex: '0 0 auto', padding: '0 16px', background: 'transparent', border: '1px solid var(--lp-border-default)', color: 'var(--lp-text-secondary)' };
  if (kind === 'danger') return { ...base, flex: '0 0 auto', padding: '0 16px', background: 'transparent', border: '1px solid color-mix(in srgb,var(--lp-danger-600) 30%,transparent)', color: 'var(--lp-danger-600)' };
...
export default function ComprasScreen({
  data = DEMO, onNuevaOC, onAprobarOC, onEditarOC, onEliminarOC, onRecibirMP, onRegistrarPago, onImprimirOC,
  ...
  const list = ocs.filter(o => o.estado === tab);
  ...
  const Row = ({ k, v, color }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
      ...
      <span style={{ color: color || 'var(--lp-text-primary)', fontWeight: 500, fontFamily: /\$|kg|L|jun|may|día/.test(String(v)) ? 'var(--lp-font-mono)' : 'inherit' }}>{v}</span>
    </div>
  );
  ...
  return (
    <div style={{ padding: pad, fontFamily: 'var(--lp-font-sans)', color: 'var(--lp-text-primary)', maxWidth: isDesktop ? 1000 : 'none', margin: '0 auto' }}>
      ...
      {list.length === 0 ? (
        <div style={{ textAlign: 'center', color: 'var(--lp-text-tertiary)', fontSize: 13.5, padding: '56px 20px' }}>Sin órdenes de compra aquí.</div>
      ) : (
              ...
              <div key={o.cod} data-id="compras.card.oc" style={{ position: 'relative', overflow: 'hidden', background: 'var(--lp-bg-raised)', border: '1px solid var(--lp-border-subtle)', borderRadius: 'var(--lp-radius-lg)', padding: '15px 16px' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {o.estado === 'activa' && <>
                    ...
                    {can('compras') && <button data-id="compras.btn.imprimir-oc" data-rol={role} onClick={() => onImprimirOC?.(o.cod)} style={btn('ghost')}><Ico d={I_PRINT} s={17} /></button>}
                  </>}
                  {o.estado === 'recibida' && <>
